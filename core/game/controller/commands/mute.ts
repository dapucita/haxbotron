            ...
            if(isNaN(target) != true && window.gameRoom.playerList.has(target) == true) {
                ...
                if(window.gameRoom.playerList.get(target)!.permissions.mute === true) {
                    ...
                    if(BotSettings.antiMuteAbusing === true) { //when anti abusing option is enabled, check the condition by compare time
                        if(mutedTimeStamp > window.gameRoom.playerList.get(target)!.permissions.muteExpire + BotSettings.muteAllowIntervalMillisecs) {
                            window.gameRoom.playerList.get(target)!.permissions.mute = true; //mute
                            window.gameRoom.playerList.get(target)!.permissions.muteExpire = mutedTimeStamp + BotSettings.muteDefaultMillisecs; //record mute expiration date by unix timestamp
