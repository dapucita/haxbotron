...
export function cmdAfk(byPlayer: PlayerObject, message?: string): void {
    ...
    if (window.gameRoom.playerList.get(byPlayer.id)!.permissions.afkmode === true) { // if this player is AFK
        ...
        if(BotSettings.antiAFKFlood === true && window.gameRoom.playerList.get(byPlayer.id)!.permissions.mute === true) {
        ...
        if(BotSettings.antiAFKAbusing === true && window.gameRoom.playerList.get(byPlayer.id)!.team !== TeamID.Spec) {
