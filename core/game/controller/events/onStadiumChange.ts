...
export function onStadiumChangeListner(newStadiumName: string, byPlayer: PlayerObject): void {
    ...
    if (byPlayer !== null && window.gameRoom.playerList.size != 0 && byPlayer.id != 0) { // if size == 0, that means there's no players. byPlayer !=0  means that the map is changed by system, not player.
        ...
        if (window.gameRoom.playerList.get(byPlayer.id)!.permissions['superadmin'] == true) {
            ...
            window.gameRoom.logger.i(`The map ${newStadiumName} has been loaded by ${byPlayer.name}#${byPlayer.id}.(super:${window.gameRoom.playerList.get(byPlayer.id)!.permissions['superadmin']})`);
            ...
            window.gameRoom.logger.i(`The map${byPlayer.name}#${byPlayer.id} tried to set a new stadium(${newStadiumName}), but it is rejected.(super:${window.gameRoom.playerList.get(byPlayer.id)!.permissions['superadmin']})`);
        ...
        window.gameRoom.logger.i(`The map ${newStadiumName} has been loaded as default map.`);
    }
}
